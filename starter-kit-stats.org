...
** Set R directory
#+src-name: r-directory
#+begin_src emacs-lisp
(setq-default inferior-R-program-name "C:\\Program Files\\R\\R-3.5.2\\bin\\x64\\Rterm.exe")
#+end_src
...
** Adds autoload for Rmd, Rnw and Snw
#+source: load-rmd
#+begin_src emacs-lisp
...
(setq ess-ask-for-ess-directory nil)
 ...
 (add-hook 'Rnw-mode-hook
  ...
  (require 'ess-site)
#+end_src
** Adds shortcut for starting R/Script

#+src-name: shortcut-start-r
#+begin_src emacs-lisp
(defun R-scratch ()
  (interactive)
  (progn
...
** Add a Keyboard Shortcut for the Pipe/Assign Operator
...
(defun my_assign_operator ()
  "R/ESS <- operator"
  (interactive)
  (insert "<- "))
(define-key ess-mode-map (kbd "M-a") 'my_assign_operator)
(define-key inferior-ess-mode-map (kbd "M-a") 'my_assign_operator)


#+end_src

** Add a Keyboard Shortcut for running Shinny app
#+src-name: ess-shiny-shortcut
#+begin_src emacs-lisp
